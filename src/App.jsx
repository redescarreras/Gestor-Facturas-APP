                  ...
                  <div className="flex-1 px-4 flex items-center text-sm text-gray-500 overflow-x-auto">
                    ...
                    {navState.empresa && (
                      <>
                        ...
                        <button onClick={() => setNavState({...navState, encargado: null})} className="hover:text-red-600 transition-colors font-medium">{navState.empresa}</button>
                      </>
                    )}
                    {navState.encargado && (
                      <>
                  ...
                  {!searchQuery && navState.empresa && (
                    <div className="bg-gray-50 border-b border-gray-200 p-4 flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <button onClick={() => setNavState({...navState, encargado: null})} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><ArrowLeft size={20} /></button>
                        <div>
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="bg-gray-50 text-gray-600 font-bold border-b border-gray-200 uppercase text-xs">
                        <tr>
                          ...
                          <th className="px-6 py-4 text-center">Total 5% Incl.</th>
                        ...
                        {obrasFiltradas.map(obra => {
                          ...
                          <tr key={obra.id} className="hover:bg-red-50/30 transition-colors group">
                            ...
                            <td className="px-6 py-4 text-center">
                              <span className={`px-2 py-1 rounded-full text-xs font-bold ${obra.estado === 'cobrado' ? 'bg-green-100 text-green-700' : obra.estado === 'facturado' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  ...
                  {Object.keys(treeData).map(empresa => (
                    <div key={empresa} onClick={() => setNavState({ ...navState, empresa })} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-red-200 cursor-pointer transition-all group relative overflow-hidden">
                      ...
                      <div className="border-t border-gray-100 pt-3">
                        ...
                        <p className="text-xl font-bold text-red-600 group-hover:text-red-700">{treeData[empresa].totalPendiente.toLocaleString()} €</p>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {Object.keys(treeData[navState.empresa]?.encargados || {}).map(encargado => (
                    <div key={encargado} onClick={() => setNavState({ ...navState, encargado })} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-200 cursor-pointer transition-all group">
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg flex justify-between items-center">
                          ...
                          <span className="text-lg font-bold text-gray-900">{treeData[navState.empresa].encargados[encargado].totalPendiente.toLocaleString()} €</span>
                        </div>
                    </div>
                  ))}
                </div>
                ...
                {ciclos.map(ciclo => (
                  <div key={ciclo.id} onClick={() => setViewCiclo(ciclo)} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-lg cursor-pointer transition-all group">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="text-xs text-gray-400 font-mono">{new Date(ciclo.fecha).toLocaleDateString()}</span>
                    </div>
                    <h4 className="font-bold text-lg text-gray-800 mb-1">{ciclo.nombre}</h4>
                    <p className="text-sm text-gray-500 mb-4">{ciclo.totalObras} expedientes</p>
                    <div className="border-t border-gray-100 pt-3">
                      <p className="text-xs text-gray-400 uppercase">Facturado</p>
                      <p className="text-xl font-bold text-gray-900">{(ciclo.obras || []).reduce((acc, o) => acc + (parseFloat(o.importe)||0), 0).toLocaleString()} €</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'cierres' && viewCiclo && (
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center no-print">
                <button onClick={() => setViewCiclo(null)} className="flex items-center gap-2 text-gray-600 hover:text-red-600 font-bold">
                         ...
                         return (
                           <tr key={o.id} className="border-b border-gray-50">
                             ...
                             <td className="py-1 px-2 flex items-center gap-2">
                               ...
                               <button onClick={() => handleUpdateFacturaCiclo(o.id, o.numFactura)} className="text-blue-600 hover:text-blue-800 p-1 no-print">
                                 <Edit size={12}/>
                               </button>
                             </td>
                             <td className="py-1 px-2">{o.central}</td>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="bg-blue-50 px-6 py-4 border-b border-blue-100">
                  <h3 className="font-bold text-blue-800 flex items-center gap-2">
                    <Database size={18}/> Copia de Seguridad y Datos
                  </h3>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                  <h3 className="font-bold text-gray-800 flex items-center gap-2">
                    <Settings size={18}/> Listas Desplegables
                  </h3>
          ...
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveObra} className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-4 md:col-span-1 border-r border-gray-100 pr-4">
                ...
                <InputGroup label="Zona / Central">
                   ...
                   <datalist id="centrales-datalist">
                     {[...(config.centrales || [])].sort((a, b) => a.localeCompare(b, 'es')).map(op => <option key={op} value={op}></option>)}
                   </datalist>
                </InputGroup>
              </div>
              <div className="space-y-4 md:col-span-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <InputGroup label="UUII (Viviendas)">
                    <input type="number" className="input-field border-blue-200 bg-blue-50 text-blue-800 font-medium" value={formData.uuii} onChange={e => setFormData({...formData, uuii: e.target.value})} placeholder="Nº..." />
                  </InputGroup>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mt-2 shadow-inner">
                   ...
                   <div className="space-y-2 text-xs text-gray-600 border-t border-gray-200 pt-2">
                      <div className="flex justify-between">
                        <span>Base:</span>
                        <span className="font-medium">{parseFloat(formData.importe || 0).toFixed(2)} €</span>
                      </div>
                      {formData.tieneRetencion && (
                        <div className="flex justify-between text-blue-600">
                          <span>+ Plus (5%):</span>
                          <span className="font-medium">+{(parseFloat(formData.importe || 0)*0.05).toFixed(2)} €</span>
                        </div>
                      )}
                      {parseFloat(formData.uuii) > 0 && (
                        <div className="flex justify-between text-purple-600">
                          <span>+ Viviendas ({formData.uuii}):</span>
                          <span className="font-medium">+{(parseFloat(formData.uuii)*1.5).toFixed(2)} €</span>
                        </div>
                      )}
                      <div className="flex justify-between font-black text-gray-900 text-lg pt-2 border-t border-gray-200 mt-2">
                        <span>TOTAL ESTIMADO:</span>
                        <span>{(parseFloat(formData.importe || 0) + (formData.tieneRetencion ? parseFloat(formData.importe || 0)*0.05 : 0) + (parseFloat(formData.uuii || 0)*1.5)).toFixed(2)} €</span>
                      </div>
                ...
                <div className="flex bg-gray-100 rounded-lg p-1 mt-2">
                  {['pendiente', 'facturado', 'cobrado'].map(st => (
                    <button type="button" key={st} onClick={() => setFormData({...formData, estado: st})} className={`flex-1 py-1 text-xs font-bold rounded capitalize ${formData.estado === st ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400'}`}>
                      {st}
                    </button>
                  ))}
                </div>
              ...
              <div className="md:col-span-3 pt-4 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" className="px-8 py-2 rounded-lg text-white font-bold shadow-lg shadow-red-200 flex items-center gap-2 bg-red-600 hover:bg-red-700 active:scale-95 transition-all">Guardar Obra</button>
              </div>
            </form>
          </div>
        </div>
